                                      ...
                                      int indexForAlpha) const
{
  ...
  if(forcefield.ewald) {
    ...
    double wolf_electrostatic = erfc(forcefield.wolfAlpha[b][indexForAlpha][indexForAlpha] * dist)/dist;
    ...
    int indexForAlpha) const
{
  ...
  if(forcefield.ewald) {
    ...
    return qi_qj * (temp / dist + constValue * expConstValue) / distSq;
      ...
      double wolf_electrostatic_force = erfc(forcefield.wolfAlpha[b][indexForAlpha][indexForAlpha] * dist)/distSq;
      // M_2_SQRTPI is 2/sqrt(PI)
      wolf_electrostatic_force += forcefield.wolfFactor3[b]*exp(-1.0*pow(forcefield.wolfAlpha[b][indexForAlpha][indexForAlpha], 2.0)*distSq)/dist;
