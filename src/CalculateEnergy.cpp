    ...
    const uint box)
{
  ...
  std::cout << "You used " << *pointerToIndexForTuple << " spaces in cuda in BoxInter" << std::endl;
  ...
  double tempREnOMP = 0.0, tempLJEnOMP = 0.0;
  tempREn = 0.0;
  tempLJEn = 0.0;
                                          ...
                                          const uint box)
{
  ...
  double tempREnCUDA = 0.0;
  double tempLJEnCUDA = 0.0;
  ...
  double tempLJEnOMP = 0.0;
  double tempREnOMP = 0.0;
...
Virial CalculateEnergy::VirialCalc(const uint box)
{
  ...
  std::cout << "You used " << *pointerToIndexForTuple << " spaces in cuda in BoxInterForce" << std::endl;
  pc.sortCUDATuples(currentParticleArray, neighborParticleArray, ljArray, *pointerToIndexForTuple);
        ...
        if(currParticle < nParticle && particleMol[currParticle] != particleMol[nParticle]) {
          if (currentAxes.InRcut(distSq, virC, currentCoords, currParticle,
            ...
            #pragma omp critical
            {
              currentParticleArrayOpenMP[indexForTupleOpenMP] = currParticle;
              neighborParticleArrayOpenMP[indexForTupleOpenMP] = nParticle;
              ljArrayOpenMP[indexForTupleOpenMP] = pVF * (virC.x * comC.x);
              indexForTupleOpenMP++;
            }
...
//#endif
  ...
  //#endif
  std::cout << "You used " << indexForTupleOpenMP << " spaces in openmp in BoxInterForce" << std::endl;
  //pc.sortOMPTuples(currentParticleArrayOpenMP, neighborParticleArrayOpenMP, ljArrayOpenMP, indexForTupleOpenMP);
/*
  if (indexForTupleOpenMP == *pointerToIndexForTuple){
    for (uint i = 0; i < pc.row_vec_omp.size(); i++){
    ...
    if(!pc.AlmostEqualUlps(rT11OMP, rT11CUDA, 1)){
      std::cout << "rT11 reductions differ in value!" << std::endl;
      std::cout << "rT11 OMP : " << rT11OMP << " != rT11 CUDA : " << rT11CUDA << std::endl;
      exit(EXIT_FAILURE);
    }
