  ...
  int *kill)
{
  ...
  */
  ...
  BufferAccess<DeviceArray<double>, double, buffers> com_x(*(vars->gpu_com_x), 0);
  ...
  if (isOrthogonal){
    BrownianMotionRotateKernel<true><<< molCount, threadsPerBlock>>>(
  ...
  int *kill)
{
  ...
  int mol = gpu_molLookup[molIndex];
  int startIdx = startAtomIdx[mol];
  ...
  if(threadIdx.x == 0) {
    com = make_double3(gpu_comx[mol], gpu_comy[mol], gpu_comz[mol]);
    // This section calculates the amount of rotation
    double stdDev = sqrt(2.0 * r_max);
    double btm_x = molTorquex[mol] * BETA * r_max;
    double btm_y = molTorquey[mol] * BETA * r_max;
    double btm_z = molTorquez[mol] * BETA * r_max;

    double3 randnums = randomGaussianCoordsGPU(mol, key, step, seed, 0.0, stdDev);
  ...
  for(atomIdx = startIdx + threadIdx.x; atomIdx < endIdx; atomIdx += blockDim.x) {
    ...
    double coor_x_com_zero = coor.x - com.x;
    double coor_y_com_zero = coor.y - com.y;
    double coor_z_com_zero = coor.z - com.z;
    ...
    //  old way used coor.x, coor.y, coor.z
    double newx = matrix[0][0] * coor_x_com_zero + 
                  matrix[0][1] * coor_y_com_zero + 
                  matrix[0][2] * coor_z_com_zero;
    double newy = matrix[1][0] * coor_x_com_zero + 
                  matrix[1][1] * coor_y_com_zero + 
                  matrix[1][2] * coor_z_com_zero;
    double newz = matrix[2][0] * coor_x_com_zero + 
                  matrix[2][1] * coor_y_com_zero + 
                  matrix[2][2] * coor_z_com_zero;

    // move back to com
    coor.x = moveMol * (newx + com.x) + !moveMol * coor.x;
    coor.y = moveMol * (newy + com.y) + !moveMol * coor.y;
    coor.z = moveMol * (newz + com.z) + !moveMol * coor.z;
...
*/
  ...
  if (isOrthogonal){
    BrownianMotionTranslateKernel<true><<< molCount, threadsPerBlock>>>(
      vars->gpu_startAtomIdx,
      gpu_molLookup,
      gpu_molBoxCount,
      old_coords_x->get(),
    ...
    BrownianMotionTranslateKernel<false><<< molCount, threadsPerBlock>>>(
      vars->gpu_startAtomIdx,
      gpu_molLookup,
      gpu_molBoxCount,
      old_coords_x->get(),
  ...
  int *kill)
{
  ...
  if(threadIdx.x == 0) {
    ...
    double bfm_x = (molForcex[mol] + molForceRecx[mol]) * BETA * t_max;
    double bfm_y = (molForcey[mol] + molForceRecy[mol]) * BETA * t_max;
    double bfm_z = (molForcez[mol] + molForceRecz[mol]) * BETA * t_max;
    ...
    gpu_t_k_x[mol] = shift.x;
    gpu_t_k_y[mol] = shift.y;
    gpu_t_k_z[mol] = shift.z;
    // shift COM
    com.x += shift.x * moveMol;
    com.y += shift.y * moveMol;
    com.z += shift.z * moveMol;
    // wrap COM
    if(isOrthogonal)
      WrapPBC3(com, axis);
    else
    ...
    //update COM
    gpu_new_comx[mol] = com.x;
    gpu_new_comy[mol] = com.y;
    gpu_new_comz[mol] = com.z;
    //check for bad configuration
  ...
  for(atomIdx = startIdx + threadIdx.x; atomIdx < endIdx; atomIdx += blockDim.x) {
    ...
    coor.y += shift.y * moveMol;
    coor.z += shift.z * moveMol;
    // wrap coordinate
    if(isOrthogonal)
      WrapPBC3(coor, axis);
    else
